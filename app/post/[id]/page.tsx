...
export default async function PostPage({ params } : PostPageProps ) {
    ...
    if (userError) {
        // Handle the error if the user doesn't exist, but still allow the page to load
        console.error('Error finding the user:', userError);
    }

    if (!user) {
        // This happens if the user ID exists in auth.users but not in the public.users table
        console.warn('User profile not found in public.users table.');
    }   

    // get the data from the join table for likes
    const { data: likeData, error: likeDataError } = await supabase.from("user_like_join").select("user_id,post_id").eq("user_id", finalPost?.creator).eq("post_id", finalPost?.id).single();

    // check for unexpected errors first
    if (likeDataError && likeDataError.code !== "PGRST116") { // Supabase 'not found' error code according to Gemini
        console.error("An unexpected error occurred while checking for a like:", likeDataError);
    }

    // if we haven't ran into any unexpected errors, then the user has liked the post if likeData exists but not if it's null
    const hasLiked = likeData ? true : false;
